$(document).ready(function() {
  ...
  // On page load, fetch the current leader
  // and update the DOM
  $.ajax({
    // method: "POST",
    url:'http://localhost:3333/leaderboards.json'
    // body: { leaderboard: { username: "Andrew"} }
  }).then(
    // Success callback fn (when the above is a 200)
    (data) => {
      $leader.text(`${data[0].username} with ${data[0].points} points`)
    },
    // Error callback fn (when the above is 4xx or 5xx)
    (error) => {
